<?php

require_once 'Controller/TechManController.php';
echo "<center>";
echo "<h1>";

echo "<br>";
echo "<a href='afmBarcode.php' target='_blank'>AFM BARCODE</a>";


 echo "<hr>";
echo "<a href='myStock.php' target='_blank'>My Stock</a>";
            
echo "<br>";
echo "<br>";
echo "<a href='http://ec2-54-74-154-166.eu-west-1.compute.amazonaws.com:8080/Pet4U/index.htm' target='_blank'>GO NEW SERVER</a>";
echo "<br>";
echo "<br>";
echo "<hr>";
echo "<hr>";
echo "<a href='https://apkfab.com/pet4u20/com.example.pet4u_20/apk?h=08b49631092b377b83abc718d8c8be1aadba93e48aef74e6a2a7165c330af593'>DOWNLOAD ANDROID APPLICATION</a>";
echo "<br>";
echo "<br>";
echo "<img src='images/qr_1.png' style='width:200px;height:200px;' >";
echo "<br>";
echo "<br>";
echo "<hr>";

echo "<br>";
echo "<br>";
echo "<br>";
echo "<hr>";
echo "<hr>";


echo "<a href='https://apkfab.com/pet4u30/com.example.pet4u_30/apk?h=e8449e2e90a83e2d230e61511f97a86c7e46ed18ea95967a0f769f3c8d18ddc6'>NEW VERSION DOWNLOAD</a>";
echo "<br>";
echo "<br>";
echo "<img src='images/qr_6.png' style='width:200px;height:200px;' >";
echo "<hr>";
echo "<hr>";
echo "<hr>";
echo "<hr>";
echo "<a href='https://appsenjoy.com/en/Nm6MpSiXDFUHQvf/file'>NEW VERSION DOWNLOAD MIRROR</a>";
echo "<br>";
echo "<br>";
echo "<img src='images/qr_7.png' style='width:200px;height:200px;' >";
echo "<hr>";
echo "<hr>";
echo "<hr>";
echo "<hr>";
echo "<a href='https://i.diawi.com/SMEXBT'>NEW VERSION DOWNLOAD MIRROR @</a>";
echo "<br>";
echo "<br>";
echo "<img src='images/qr_8.png' style='width:200px;height:200px;' >";
echo "</center>";

exit;
//echo "<a href='addNewItem.php'>Add New Item</a>";
//echo "<br>";
//echo "<a href='items.php'>All Items</a>";
//echo "<br>";
//echo "<a href='upload.php'>Upload Items From Excel File</a>";
//echo "<br>";
echo "<a href='itemFinder.php'>Find Item</a>";
//echo "<br>";
//echo "<a href='download.php'>DOWNLOAD</a>";
echo "<br>";
echo "<a href='lastScanned.php'>LAST SCANNED BARCODES</a>";

//echo "<br>";
//echo "<a href='allPositions.php' target='_blank'>SHOW ALL ITEMS BY POSITION</a>";
echo "<br>";
echo "<a href='salesByPosition.php' target='_blank'>SHOW SALES BY POSITION</a>";
echo "<br>";
echo "<a href='showSales.php' target='_blank'>SHOW SALES BY POSITION(Items without sales not included)</a>";
echo "<br>";
echo "<a href='salesFinder.php'>Find Item Sales</a>";
echo "<br>";
echo "<a href='massScan.php'>Mass Scan</a>";
echo "<hr>";
echo "<br>";
echo "<a href='upload.php'>Upload</a>";

echo "<hr>";
echo "<br>";
echo "<a href='uploadSales.php'>Upload Sales</a>";
echo "<hr>";
echo "<br>";


echo "</h1>";
?>